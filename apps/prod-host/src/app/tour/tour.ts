...
const BASICS_STEPS: readonly TourStep[] = [
  ...
  },
  {
    ...
    body:
      'Built separately, deployed separately, fetched over the network at runtime, and rendered here. It shares no code with this page except the `@skew/core` runtime itself — the two builds agree on a storage envelope, nothing more.',
  },
  {
    id: 'store',
    route: '/basics',
...
const PORTFOLIO_STEPS: readonly TourStep[] = [
  ...
  },
  {
    ...
    body:
      'Submitting an order goes through the @skew/angular-data outbox. The API genuinely refuses a v1-shaped order with 409 version-skew, and the runner catches that, migrates the queued payload, and retries — rather than the queue resending the same stale envelope forever.',
