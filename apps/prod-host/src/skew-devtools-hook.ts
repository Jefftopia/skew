 ...
 *
 * `@skew/core` emits one trace event per `read()` / `write()` into
 * `globalThis.__SKEW_DEVTOOLS_HOOK__` when one is installed — and because the
 * host and the remote share a single `@skew/core` instance via federation's
 ...
 * of `@skew/core` into the host entry and break the singleton the whole demo
