...
A **protections switch** at the top of the host makes the `@braidlabs` packages
...
The engine is `diffPayloads` from **`@braidlabs/studio`** — the first shipped
...
installs the `@braidlabs/skew` trace hook (`__SKEW_DEVTOOLS_HOOK__`, in
...
the order goes through the `@braidlabs/angular-data` outbox, and `/api/v2/orders`
