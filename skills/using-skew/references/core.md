# @skewkit/core — versioned schemas, storage, and build-skew detection
...
|---|---|---|
| `ahead` | written by a NEWER build; steps to read it don't exist here | refetch from server, leave data untouched, or cure via `@skewkit/contract` resolver — **never discard** |
...
```ts
import { createVersionedStore, webStorageDriver, memoryDriver } from '@skewkit/core';
...
const probe = createVersionProbe({
  identity: { buildId: BUILD_ID, builtAt: BUILT_AT },  // from @skewkit/build
