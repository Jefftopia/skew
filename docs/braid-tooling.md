...
| `@braidlabs/cli/nx` | **works** | Nx plugin inferring a `braid-dev` target from any `braid.config.*` |
...
$ braid add-gateway
  ...
  → package.json               added @braidlabs/gateway
```
...
### `braid ng-add` (or `nx g @braidlabs/angular:setup`)
