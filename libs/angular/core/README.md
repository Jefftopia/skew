...
First-class Angular bindings for `@braidlabs/skew`.

This package provides the baseline Dependency Injection (DI) and reactive Signal wrappers needed to integrate `@braidlabs/skew`'s versioned stores seamlessly into an Angular application.
...
`@braidlabs/skew` operations return `Promise<SkewResult<T>>`. While this works perfectly for standard async flows, it can cause a flash of empty state in the UI if you are using a synchronous storage driver (like `localStorage`).
