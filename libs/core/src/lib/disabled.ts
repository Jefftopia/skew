 ...
 *
 * A process-wide switch that makes every `@skewkit` package behave as though it
 * were not installed:
 *
 ...
 *
 * `@skewkit/core` has no framework and no dependencies, so there is no injector to
 ...
 * instance*, so it only behaves as one switch when `@skewkit/core` is genuinely
