 ...
 *
 * A process-wide switch that makes every `@skew` package behave as though it
 * were not installed:
 *
 ...
 *
 * `@skew/core` has no framework and no dependencies, so there is no injector to
 ...
 * instance*, so it only behaves as one switch when `@skew/core` is genuinely
