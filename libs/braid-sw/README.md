...
The queue is `@braidlabs/data`'s outbox rather than a second implementation: one record per entry, so
...
**Cache API for assets and stubs. IndexedDB (`@braidlabs/data`) for entities.**
