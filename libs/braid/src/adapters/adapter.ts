    ...
    throw new BraidError(`no adapter named "${adapterName}" is installed in this client`, {
      ...
      fixHint:
        adapterName === DEFAULT_ADAPTER
          ...
          : `this build of @skewkit/braid ships only the "compat" adapter; either set "adapter": "compat" in the fragment's manifest (or omit it — compat is the default), or install a client build that provides "${adapterName}"`,
    });
  }
  return adapter;
}
