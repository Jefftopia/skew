...
export interface ServiceWorkerOptions {
   ...
   * Defaults to `/braid-sw.js`, which is where a build that copies `@braid/sw` normally
   * lands it.
   */
  module?: string;
...
export function createGateway(options: GatewayOptions): BraidGateway {
          ...
          `<!doctype html><title>Braid: unknown fragment</title>` +
            (mode === 'development'
              ? `<p>braid-gateway: no fragment with id "${escapeHtml(route.fragmentId)}" is registered.<br>` +
                `Register a manifest for it in the gateway registry, and ensure @braid/core and @braid/gateway versions match.</p>`
  ...
  function serviceWorkerResponse(config: ServiceWorkerOptions): Response {
    ...
    const script =
      `// Generated by @braid/gateway. Serves the Braid namespace only.\n` +
